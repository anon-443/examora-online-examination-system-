...
export function PracticeLab() {
  ...
  const { data: attemptHistory = [] } = trpc.attempts.history.useQuery(undefined, { enabled: Boolean(user) });
  const startPersistedAssessment = trpc.attempts.start.useMutation({ onError: error => toast.error(error.message) });
  const savedPractice = attemptHistory.filter(attempt => attempt.status === "submitted").slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--page))]">
      <SiteHeader />
      <main className="container py-9 sm:py-18">
        <section className="grid gap-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3"><Button disabled={startPersistedAssessment.isPending} onClick={launchPersistedPractice} className="min-h-11 rounded-xl bg-slate-950 text-white hover:bg-slate-800 dark:bg-white dark:text-slate-950">{startPersistedAssessment.isPending ? "Preparing practice…" : user ? "Launch saved practice" : "Sign in to save practice"}<ArrowRight className="ml-1.5 size-4" /></Button><Button asChild variant="outline" className="min-h-11 rounded-xl"><Link href="/exams">Browse available exams</Link></Button><Button asChild variant="ghost" className="min-h-11 rounded-xl"><Link href="/study-hub">Need a study cue?</Link></Button></div>
          </div>
          <div className="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-7 shadow-[0_24px_60px_-35px_rgba(15,23,42,0.45)] sm:p-9 dark:border-white/10 dark:bg-slate-900">
            <img src="/manus-storage/examora-subject-maths-science_93489760.png" alt="" loading="lazy" className="absolute inset-0 h-full w-full object-cover opacity-[0.09] dark:opacity-[0.14]" />
            <div className="absolute inset-0 bg-gradient-to-br from-white/75 via-white/55 to-emerald-50/35 dark:from-slate-900/85 dark:via-slate-900/72 dark:to-emerald-950/35" />
            ...
            <div className="relative mt-6 grid grid-cols-3 gap-2">{[10, 20, 30].map((option) => <button key={option} onClick={() => setMinutes(option)} className={`min-h-12 rounded-xl border px-3 py-3 text-sm font-bold transition-all ${minutes === option ? "border-emerald-600 bg-emerald-600 text-white shadow-lg shadow-emerald-600/20" : "border-slate-200 text-slate-600 hover:border-emerald-300 dark:border-white/10 dark:text-slate-300"}`}>{option} min</button>)}</div>
            <div className="relative mt-5 flex rounded-xl bg-slate-100 p-1 dark:bg-white/[0.06]">{(["Focus", "Review"] as const).map((option) => <button key={option} onClick={() => setMode(option)} className={`min-h-10 flex-1 rounded-lg py-2 text-sm font-bold transition-colors ${mode === option ? "bg-white text-slate-950 shadow-sm dark:bg-slate-800 dark:text-white" : "text-slate-500 dark:text-slate-400"}`}>{option} mode</button>)}</div>
        ...
        <section className="visual-accent-card mt-10 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:mt-14 sm:p-8 dark:border-white/10 dark:bg-slate-900">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-emerald-700 dark:text-emerald-400">Saved practice outcomes</p><h2 className="mt-2 font-display text-3xl font-semibold tracking-[-0.05em] text-slate-950 dark:text-white">Review your completed MCQs in detail</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-400">Every saved assessment retains your score, answer-by-answer feedback, explanations, and a next-step summary.</p></div><Button asChild variant="outline" className="min-h-11 rounded-xl"><Link href="/history">All activity <ArrowRight className="ml-1.5 size-4" /></Link></Button></div>
          <div className="mt-6 grid gap-4 md:grid-cols-3">{savedPractice.length ? savedPractice.map(attempt => <article key={attempt.id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-5 dark:border-white/10 dark:bg-white/[0.035]"><div className="flex items-start justify-between gap-3"><Badge className="rounded-full border-0 bg-emerald-100 text-emerald-800 dark:bg-emerald-400/15 dark:text-emerald-300">{attempt.subject}</Badge><span className={`text-sm font-bold ${attempt.percentage >= 65 ? "text-emerald-700 dark:text-emerald-300" : "text-amber-700 dark:text-amber-300"}`}>{attempt.percentage}%</span></div><h3 className="mt-4 font-display text-lg font-semibold text-slate-950 dark:text-white">{attempt.examTitle}</h3><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">{attempt.score}/{attempt.totalQuestions} correct · detailed feedback ready</p><Button asChild variant="outline" className="mt-5 min-h-10 w-full rounded-xl"><Link href={`/results/${attempt.id}`}>Open detailed result <ArrowRight className="ml-1.5 size-4" /></Link></Button></article>) : <div className="md:col-span-3 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-8 text-center dark:border-white/15 dark:bg-white/[0.03]"><CheckCircle2 className="mx-auto size-6 text-emerald-600" /><p className="mt-3 font-semibold text-slate-900 dark:text-white">Your next saved practice result will appear here</p><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Launch a saved practice assessment to receive a score and explanation-aware review.</p></div>}</div>
        </section>
