  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[hsl(var(--page))] text-slate-950 dark:text-slate-50">
      ...
      <main>
        ...
        <section className="section-wash py-10 sm:py-12"><div className="container"><SectionHeading eyebrow="Your path" title="Three steps to a clearer result" /><div className="mt-7 grid gap-5 md:grid-cols-3">{pathway.map(([number, title, copy]) => <article key={number} className="interactive-card flex min-h-[255px] flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white p-7 text-center shadow-sm dark:border-white/10 dark:bg-slate-900"><span className="grid size-12 place-items-center rounded-full bg-slate-950 text-sm font-bold text-white dark:bg-emerald-400 dark:text-slate-950">{number}</span><h3 className="mt-6 font-display text-2xl font-semibold text-slate-950 dark:text-white">{title}</h3><p className="mt-3 max-w-sm text-base leading-7 text-slate-600 dark:text-slate-400">{copy}</p></article>)}</div></div></section>
        ...
        <section className="container py-12 sm:py-16"><SectionHeading eyebrow="Learning experience" title="Designed for focused progress" /><div className="mt-8 grid gap-5 md:grid-cols-3">{capabilityCards.map(([Icon, title, copy]) => { const CapabilityIcon = Icon as typeof Clock3; return <article key={String(title)} className="interactive-card rounded-2xl border border-slate-200 bg-white p-7 shadow-sm dark:border-white/10 dark:bg-slate-900"><CapabilityIcon className="size-6 text-emerald-600 dark:text-emerald-300" /><h3 className="mt-6 font-display text-2xl font-semibold text-slate-950 dark:text-white">{String(title)}</h3><p className="mt-3 text-base leading-7 text-slate-600 dark:text-slate-400">{String(copy)}</p></article>; })}</div></section>
