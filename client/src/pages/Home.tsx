  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[hsl(var(--page))] text-slate-950 dark:text-slate-50">
      ...
      <main>
        ...
        <section className="section-wash py-12 sm:py-18"><div className="container"><SectionHeading eyebrow="Your path" title="Three steps to a clearer result" /><div className="mt-8 grid gap-4 md:grid-cols-3">{pathway.map(([number, title, copy]) => <article key={number} className="interactive-card rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900"><span className="grid size-10 place-items-center rounded-full bg-slate-950 text-xs font-bold text-white dark:bg-emerald-400 dark:text-slate-950">{number}</span><h3 className="mt-6 font-display text-xl font-semibold text-slate-950 dark:text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">{copy}</p></article>)}</div></div></section>

        <section className="container py-12 sm:py-18"><SectionHeading eyebrow="The catalogue" title="What will you be assessed on" /><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{subjects.map(({ name, icon: Icon, accent }) => <Link key={name} href="/exams" className="interactive-card group relative flex min-h-[190px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-slate-900"><div className={`absolute inset-0 bg-gradient-to-br ${accent} opacity-0 transition-opacity group-hover:opacity-100`} /><span className="relative grid size-10 place-items-center rounded-xl bg-emerald-50 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-300"><Icon className="size-5" /></span><h3 className="relative mt-5 font-display text-xl font-semibold text-slate-950 dark:text-white">{name}</h3><span className="relative mt-auto inline-flex items-center text-sm font-bold text-emerald-700 dark:text-emerald-300">Explore <ChevronRight className="ml-1 size-4 transition-transform group-hover:translate-x-1" /></span></Link>)}</div></section>
