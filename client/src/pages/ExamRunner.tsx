...
export default function ExamRunner() {
  ...
  if (!session) return <div className="min-h-screen bg-slate-50 dark:bg-slate-950"><SiteHeader /><div className="container py-20 text-center"><h1 className="font-display text-3xl font-semibold text-slate-950 dark:text-white">This assessment is no longer active.</h1><Button className="mt-5 rounded-xl" onClick={() => setLocation("/exams")}>Return to catalogue</Button></div><SiteFooter /></div>;
  ...
  return <div className="min-h-screen bg-slate-50 dark:bg-slate-950"><SiteHeader /><StudentGate><main className="container py-6 sm:py-10"><div className="mx-auto max-w-6xl">
    <header className="flex flex-col gap-5 border-b border-slate-200 pb-5 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><Badge className="rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-400/15 dark:text-emerald-300">{session.exam.subject}</Badge><span className="text-xs font-semibold text-slate-400">Question {activeIndex + 1} of {session.questions.length}</span></div><h1 className="mt-2 font-display text-2xl font-semibold tracking-[-0.045em] text-slate-950 sm:text-3xl dark:text-white">{session.exam.title}</h1><p className="mt-2 flex items-center gap-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-400"><Cloud className="size-3.5" />{saveMutation.isPending ? "Saving answer…" : lastSuccessfulSaveAt ? `Last successful auto-save: ${new Date(lastSuccessfulSaveAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}` : "Progress is saved automatically"}</p></div><div className={`flex items-center gap-3 self-start rounded-xl border px-3.5 py-2.5 ${lowTime ? "border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-400/30 dark:bg-rose-400/10 dark:text-rose-300" : "border-slate-200 bg-white text-slate-800 dark:border-white/10 dark:bg-slate-900 dark:text-white"}`}><Clock3 className="size-[18px]" /><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] opacity-60">Time remaining</p><p className="font-mono text-lg font-bold tracking-tight">{formatTime(secondsLeft)}</p></div></div></header>
