...
export default function ExamRunner() {
  ...
  return <div className="min-h-screen bg-slate-50 dark:bg-slate-950"><SiteHeader /><StudentGate><main className="exam-runner container py-4 sm:py-5"><div className="mx-auto max-w-6xl">
    <header className="flex flex-col gap-3 border-b border-slate-200 pb-3 dark:border-white/10 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><Badge className="rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-400/15 dark:text-emerald-300">{session.exam.subject}</Badge><span className="text-xs font-semibold text-slate-400">Question {activeIndex + 1} of {session.questions.length}</span></div><h1 className="mt-2 font-display text-2xl font-semibold tracking-[-0.045em] text-slate-950 sm:text-3xl dark:text-white">{session.exam.title}</h1><p className="mt-1.5 flex items-center gap-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-400"><Cloud className="size-3.5" />{saveMutation.isPending ? "Saving answer…" : lastSuccessfulSaveAt ? `Last successful auto-save: ${new Date(lastSuccessfulSaveAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}` : "Progress is saved automatically"}</p></div><div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:justify-end"><div className={`flex min-w-[152px] items-center gap-2 rounded-xl border px-3 py-2 sm:min-w-[164px] ${lowTime ? "border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-400/30 dark:bg-rose-400/10 dark:text-rose-300" : "border-slate-200 bg-white text-slate-800 dark:border-white/10 dark:bg-slate-900 dark:text-white"}`}><Clock3 className="size-4" /><div><p className="text-[9px] font-bold uppercase tracking-[0.12em] opacity-60">Time remaining</p><p className="font-mono text-base font-bold tracking-tight">{formatTime(secondsLeft)}</p></div></div><div className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-right dark:border-white/10 dark:bg-slate-900"><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-slate-400">Answered</p><p className="text-sm font-bold text-slate-800 dark:text-white">{answeredCount}/{session.questions.length}</p></div></div></header>
